...
export default function Home() {
  ...
  const handleSniff = useCallback(async () => {
    ...
    try {
      ...
      setSniffResult({ ok: false, assets: [], pageTitle: '', error: 'Sniff failed.' });
    } finally {
  ...
  const spoofedUA = UA_PROFILES[uaKey]?.['User-Agent'] ?? '';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden', position: 'relative', zIndex: 1 }}>
      ...
      {loading && (
        <div style={{ height: 2, background: 'var(--border)', flexShrink: 0, overflow: 'hidden' }}>
          <div style={{ height: '100%', background: 'var(--accent)', animation: 'loadBar 1.8s ease-in-out infinite', boxShadow: '0 0 8px var(--accent-glow)' }} />
        </div>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
          {!currentUrl
            ...
            : <BrowserFrame ref={frameRef} currentUrl={currentUrl} onLoadStart={() => setLoading(true)} onLoadEnd={() => setLoading(false)} />
          }
          {sniffOpen && (
            <MediaSnifferPanel
              result={sniffResult}
        ...
        {sidebarOpen && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 40, background: 'rgba(0,0,0,0.55)' }}
            onClick={() => setSidebarOpen(false)} />
        )}
        <div style={{
          position: 'absolute', top: 0, right: 0, bottom: 0,
          width: 'min(300px, 92vw)', zIndex: 41,
          transform: sidebarOpen ? 'translateX(0)' : 'translateX(100%)',
          transition: 'transform 0.22s ease',
          display: 'flex', flexDirection: 'column',
        }}>
          <Sidebar
            jobs={jobs} onRemove={removeJob} onClearCompleted={clearCompleted}
            incognito={incognito} onToggleIncognito={handleToggleIncognito}
            spoofedUA={spoofedUA} uaKey={uaKey}
            proxyEnabled={false} onToggleProxy={() => {}}
            onClose={() => setSidebarOpen(false)}
          />
        </div>
      </div>

      <style>{`@keyframes loadBar { 0%{width:0;margin-left:0} 50%{width:55%;margin-left:20%} 100%{width:0;margin-left:100%} }`}</style>
    </div>
  );
}
...
function EmptyState({ onNavigate }: { onNavigate: (url: string) => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 5, background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', gap: 24, overflowY: 'auto' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 64, height: 64, borderRadius: 14, background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 900, fontFamily: 'var(--font-display)', color: '#000', margin: '0 auto 12px', boxShadow: '0 0 28px var(--accent-glow)' }}>Y</div>
      ...
      <div style={{ width: '100%', maxWidth: 500 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <input className="yspb-input" value={query} onChange={e => setQuery(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && query.trim() && onNavigate(query.trim())}
            placeholder="Search anything or enter a URL…" autoFocus style={{ fontSize: 16, padding: '12px 14px' }} />
          <button className="btn-primary" onClick={() => query.trim() && onNavigate(query.trim())} style={{ fontSize: 14, padding: '0 22px', flexShrink: 0 }}>GO</button>
        </div>
        <div style={{ fontSize: 10, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)', marginTop: 6, textAlign: 'center' }}>
          Searches DuckDuckGo · or enter a full URL to navigate
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 440 }}>
        {quickLinks.map(l => (
          <button key={l.url} className="btn-ghost" onClick={() => onNavigate(l.url)} style={{ fontSize: 12 }}>{l.label}</button>
        ))}
      </div>
